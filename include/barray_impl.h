...
struct barray {
	...
	void operator=(const barray_expr<T> & rhs) {
		...
		// assert(current_device == current_storage && "Array storage and device doesnot matched");

		if (current_device == DEVICE_GPU && !device_allocated)
		{
			assert(false && "need allocate array on device");
		}
...
struct barray_expr_array: public barray_expr<T>{
	...
	const builder::dyn_var<T> get_value_at(std::vector<builder::dyn_var<int>*> indices) const {
		...
		if(m_array.is_constant == 1)
		{
			return m_array.constant_val;
		}
		assert(current_device==m_array.current_storage && "Array device and storage dont match");
		if (current_device == DEVICE_HOST)
